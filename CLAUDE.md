...
- **users**: `id`, `full_name`, `email` (UNIQUE), `password_hash`, `role` ENUM(`ADMIN`,`MENTOR`,`INTERN`) default `INTERN`, `status` ENUM(`ACTIVE`,`LOCKED`) default `ACTIVE`, `avatar_url` (nullable), `deleted_at` (nullable — soft delete), `created_at`, `updated_at`.
  - Hồ sơ Intern (tất cả nullable, chỉ có ý nghĩa với `role=INTERN` — sửa qua `PATCH /users/{id}/profile`, quyền MENTOR): `bio`, `github_url`, `score` NUMERIC(5,2), `attendance_rate` NUMERIC(5,2).
    - **Đã bỏ khỏi bảng**: `mentor_id`, `phone`, `start_date`, `end_date`, `university` (migration `d5c8a2e64f19`), `major` (`e7a4b1d09c53`) và `department` — Khối kỹ thuật (`f1c6b83ad74e`). Đừng thêm lại nếu không có yêu cầu rõ ràng: cột không có form nhập thì chỉ toàn NULL, và frontend lại phải bịa giá trị mặc định để hiển thị.
  ...
  - ENUM **`department`** (giờ chỉ còn `modules.track` và `projects.department` dùng — `users.department` đã bỏ): `Java Back-End`, `React Front-End`, `Cloud & DevOps`, `Salesforce/ERP`, `AI & Data Science`. Giá trị enum = đúng nhãn frontend hiển thị (đã chốt với FE) → không cần lớp map.
...
- **Phiên đăng nhập RESET vào đúng `SESSION_RESET_HOUR_LOCAL` giờ mỗi ngày** (mặc định 00:00 giờ Việt Nam — `SESSION_RESET_UTC_OFFSET_HOURS=7`), xem `security.refresh_token_expires_at()`. Đây KHÔNG phải "N ngày kể từ lúc đăng nhập": mọi người cùng bị đăng xuất tại một mốc trong ngày, nên đăng nhập 23:50 thì phiên chỉ còn 10 phút (đúng ý đồ). `/auth/refresh` chỉ cấp access token mới, không đẩy mốc này ra xa. Lưu ý: `deps.get_current_user` chỉ verify JWT (30 phút) nên có "grace" tối đa `ACCESS_TOKEN_EXPIRE_MINUTES` sau mốc reset — access token cấp sát mốc vẫn dùng được tới khi tự hết hạn; việc đăng xuất đúng giờ do client tự làm theo `session_expires_at`.
...
- **Ai đăng nhập lần đầu cũng là `INTERN`/`ACTIVE`** — tên miền email KHÔNG quyết định vai trò. Có hai đường đổi vai trò về sau:
  - **Người dùng tự xin**: `POST /role-requests` rồi ADMIN duyệt. Hạ từ MENTOR về INTERN thì áp dụng ngay, không cần duyệt.
  - **ADMIN tự tay đặt**: `PATCH /users/{id}/role` (`INTERN` ↔ `MENTOR`, cả hai chiều, không cần ai xin trước). Không đổi được vai trò của chính mình hay của một ADMIN khác. Nếu người đó đang có yêu cầu `PENDING` thì yêu cầu được đóng trong cùng transaction để hàng đợi không kẹt.
