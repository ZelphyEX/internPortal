...
class DocumentOut(BaseModel):
    ...
    #: Một tài liệu có thể thuộc nhiều danh mục cùng lúc.
    categories: list[str] = []
...
class DocumentCreate(BaseModel):
    ...
    type: DocumentType
    #: Gửi kể cả rỗng ([]) — không gửi field này thì tài liệu không thuộc danh
    #: mục nào, khác với "chưa quyết định".
...
class DocumentUpdate(BaseModel):
    """PATCH — only provided fields change. `categories`/tag_ids present (even
    []) replaces the whole list — không có kiểu "thêm/bớt một phần tử"."""
